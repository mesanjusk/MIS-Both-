...
const escapeRegexLiteral = (value) => String(value || '').replace(/[.*+?^${}()|[\]\\]/g, '\\const roundMoney = (value) => Number(Number(value || 0).toFixed(2));');
...
async function resolveAssignedCounterparty(value) {
  ...
  const customers = await Customer.find(
    { Customer_name: { $regex: new RegExp(`^${escapeRegexLiteral(raw)}const express = require('express');
const router = express.Router();
const { requireAuth } = require('../middleware/auth');
const { v4: uuid } = require('uuid');
const multer = require('multer');
