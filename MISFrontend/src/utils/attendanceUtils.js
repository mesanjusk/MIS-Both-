...
export const calculateWorkingHours = (inTime, outTime, breakTime, startTime) => {
  ...
  const inD = parseTime(inTime), outD = parseTime(outTime), breakD = parseTime(breakTime), startD = parseTime(startTime);
...
export const processAttendanceDataRange = (records, userLookup, startISO, endISO, forcedUserName = null) => {
  ...
  records.forEach(({ Date: recDate, User, Employee_uuid, Source: recordSource }) => {
    if (!recDate) return;
    const dateISO = indiaDateISO(recDate);
    if (!dateISO || (startISO && dateISO < startISO) || (endISO && dateISO > endISO)) return;
    const employeeUuid = (Employee_uuid || "").trim();
    const user = userLookup[employeeUuid] || {};
    const name = user.name || "Unknown";
    if (forcedUserName && name !== forcedUserName) return;

    // One row = one employee + one India calendar date. Duplicate attendance
    // documents/check-ins therefore cannot inflate monthly or FY present counts.
    const identity = user.uuid || employeeUuid || name;
    const key = `${identity}-${dateISO}`;
    if (!grouped.has(key)) grouped.set(key, {
      DateISO: dateISO, Date: formatDateDMY(dateISO), User_uuid: user.uuid || employeeUuid,
      User_name: name, User_group: user.group || "", Amount: Number(user.amount || 0),
      AccountID: user.accountId || "", In: "N/A", Break: "N/A", Start: "N/A", Out: "N/A",
      TotalHours: "0.00", Late: false, HalfDay: false, Source: (recordSource || "").trim() || "",
    });
    ...
    const resolvedSource = normalizeSource(entryWithSource?.Source || entryWithSource?.source || recordSource || ref.Source);
    if (resolvedSource) ref.Source = resolvedSource;

    (User || []).forEach((u) => {
      const value = u.Time?.trim() || "N/A";
      if (u.Type === "In") {
        const current = timeValue(ref.In), candidate = timeValue(value);
        if (current === null || (candidate !== null && candidate < current)) ref.In = value;
      }
      if (u.Type === "Break" && ref.Break === "N/A") ref.Break = value;
      if (u.Type === "Start") ref.Start = value;
      if (u.Type === "Out") {
        const current = timeValue(ref.Out), candidate = timeValue(value);
        if (current === null || (candidate !== null && candidate > current)) ref.Out = value;
      }
    });
  });
  ...
  return Array.from(grouped.values()).map((r) => {
    ...
    const inTime = parseTime(r.In); const lateLimit = new Date(); lateLimit.setHours(10, 15, 0, 0);
    return { ...r, TotalHours: hours.toFixed(2), Late: inTime ? inTime > lateLimit : false,
      HalfDay: hours > 0 && hours < 4.5, Source: r.Source || "Dashboard" };
  });
};

export const processAttendanceDataForDate = (records, userLookup, dateISO) =>
  processAttendanceDataRange(records, userLookup, dateISO, dateISO);
